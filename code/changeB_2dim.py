...
OUT_CSV = OUTPUT_DIR / "changeB_2dim.csv"
...
def main():
    ...
    for B_ in B_values:
        ...
        for time_ in range(sim):
            print(f"Current simulation: {time_}, B = {B_}")
            ...
            S_df = pd.DataFrame(S_array, columns=[f"x{i+1}" for i in range(d)])
            S_df["a"] = A_vals
            S_df["y"] = Y_vals
            ...
            T_df = pd.DataFrame(T_array, columns=[f"x{i+1}" for i in range(d)])
