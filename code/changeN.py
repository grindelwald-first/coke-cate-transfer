...
OUT_CSV = OUTPUT_DIR / "changeN_seed.csv"


############################################################
# 1) Global parameters
############################################################
B = 10                 # shift between source/target
R = 2                  # shift between treatment/control (propensity)
c_val = 1              # complexity factor for or0, or1
d = 4                  # dimension
beta = 1               # number of shifted covariates
sd = 0.5               # sd of Y|A,X
sim = 100              # simulation times
nnew = 10000           # number of new samples from T for MSE

nt_values = [250, 500, 750, 1000, 1250, 1500]
...
############################################################
# 3) Helper functions: ps, or0, or1
############################################################
def expit(x):
    return 1.0 / (1.0 + np.exp(-x))

def ps(x):
    # ps(x) = expit(sum(x[0:4]) / 8 * R)
...
############################################################
# 4) Main simulation loop (vary nt)
############################################################
def main():
    results_df = pd.DataFrame(columns=["nt", "method", "risk"])
    ...
    for nt in nt_values:
        ...
        # p controls covariate shift between S and T
        p_shift = 1.0 / (1.0 + B ** (1.0 / beta))
